...
CREATE TABLE mascotas (
    ...
    fecha_nacimiento DATE,
    id_dueno INT,
    FOREIGN KEY (id_dueno) REFERENCES duenos(id_dueno) ON DELETE RESTRICT ON UPDATE RESTRICT
);
...
CREATE TABLE historiales_clinicos (
    ...
    descripcion VARCHAR(250) NOT NULL,
    id_mascota INT,
    FOREIGN KEY (id_mascota) REFERENCES mascotas(id_mascota) ON DELETE RESTRICT ON UPDATE RESTRICT,
    id_veterinario INT,
    FOREIGN KEY (id_veterinario) REFERENCES veterinarios(id_veterinario) ON DELETE RESTRICT ON UPDATE RESTRICT
);
...
FROM
    mascotas AS m
    INNER JOIN duenos AS d ON m.id_dueno = d.id_dueno -- Ejercicio 10 – JOIN múltiple con historial
